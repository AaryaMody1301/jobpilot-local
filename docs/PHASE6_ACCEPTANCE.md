...
- A Phase 6 target must be absolute HTTP(S), credential-free, and resolve syntactically to `localhost` or a loopback IP. Browser routing blocks non-loopback HTTP(S) subresources and redirects are revalidated.
...
- Pre-submit browser/network failures use a small bounded retry only for localhost fixture reliability; after the limit the attempt fails safely.
...
2. `scripts/phase6_application_acceptance.py` runs one localhost-only Playwright fixture server covering successful review/fill/confirm, unknown mandatory question while other work continues, CAPTCHA blocking, pre-submit network retry, ambiguous post-submit `UNCERTAIN`, single-worker submission, and persistent profile creation.
...
Acceptance is complete only after the exact Phase 6 branch head passes the Windows workflow and the final pull-request head passes all inherited Phase 0-6 workflows.
