...
def test_model_controller_launches_idle_and_only_detects_local_hardware(tmp_path: Path) -> None:
    paths = ManagedPaths(tmp_path / "JobPilotLocal")
    controller = ModelController(paths, MIGRATIONS, sample_item_seconds=0.01)
    try:
...
def test_model_mutations_are_blocked_while_sample_session_is_running(tmp_path: Path) -> None:
    controller = ModelController(ManagedPaths(tmp_path / "JobPilotLocal"), MIGRATIONS, sample_item_seconds=0.2)
    try:
        controller.start()
...
def test_resume_gate_remains_independent_and_not_auto_satisfied(tmp_path: Path) -> None:
    controller = ModelController(ManagedPaths(tmp_path / "JobPilotLocal"), MIGRATIONS, sample_item_seconds=0.01)
    try:
        state = controller.snapshot()
        assert state["resume"]["onboarding_ready"] is False
        assert state["model"]["human_review_gate_required"] is True
    finally:
        controller.close()
