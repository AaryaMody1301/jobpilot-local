...
def test_phase4_migration_upgrades_phase3_database_and_preserves_state(tmp_path: Path) -> None:
    ...
    with Database(database_path, MIGRATIONS) as db:
        assert db.apply_migrations() == ["005_phase4_tailoring.sql", "006_phase4_tailoring_safety.sql", "007_phase5_jobs.sql", "008_phase6_applications.sql", "009_phase8_orchestration.sql"]
