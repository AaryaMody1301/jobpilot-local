...
def _seed(db: Database) -> None:
    ...
    with db.transaction() as connection:
        connection.execute(
            "INSERT INTO runtime_sessions(id, state, started_at) VALUES ('session-1', 'running', ?)",
            (now,),
        )
        connection.execute(
            "INSERT INTO work_items(id, kind, state, lease_owner, updated_at) VALUES ('work-1', 'generation', 'running', 'session-1', ?)",
            (now,),
        )
        for app_id, identity, state in (
            ("app-submit", "job-submit", "submitting"),
            ("app-confirm", "job-confirm", "confirming"),
            ("app-fill", "job-fill", "filling"),
        ):
            connection.execute(
                "INSERT INTO application_attempts(id, job_identity, state, updated_at) VALUES (?, ?, ?, ?)",
                (app_id, identity, state, now),
            )
...
def test_unclean_recovery_requeues_only_pre_submit_work(tmp_path: Path) -> None:
    with Database(tmp_path / "db.sqlite3", MIGRATIONS) as db:
        db.apply_migrations()
        _seed(db)
        result = db.recover_interrupted_work()
        assert result == {
            "crashed_sessions": 1,
            "requeued_work": 1,
            "uncertain_applications": 2,
        }
...
def test_database_state_transition_rejects_retry_after_uncertain(tmp_path: Path) -> None:
    with Database(tmp_path / "db.sqlite3", MIGRATIONS) as db:
        ...
        db.connection.execute(
            "INSERT INTO application_attempts(id, job_identity, state, updated_at) VALUES ('app-1', 'job-1', 'uncertain', ?)",
            (now,),
        )
        with pytest.raises(InvalidTransition):
            db.journal_application_transition(
                "app-1",
                ApplicationState.QUEUED,
                reason="must never auto-retry uncertain",
            )
