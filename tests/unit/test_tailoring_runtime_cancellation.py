...
from jobpilot.model.inference import (
...
def test_inference_runtime_cancellation_reason_prefers_explicit_cancel() -> None:
    cancel = threading.Event()
    cancel.set()
    assert _runtime_cancellation_reason(cancel, _Watcher(True)) == "resume tailoring cancelled"


def test_inference_runtime_cancellation_reason_reports_memory_pressure() -> None:
    cancel = threading.Event()
...
def test_inference_runtime_cancellation_reason_preserves_other_runtime_errors() -> None:
    cancel = threading.Event()
    assert _runtime_cancellation_reason(cancel, _Watcher(False)) is None


def test_inference_timeout_scales_for_slow_cpu_prompt_and_generation() -> None:
...
def test_inference_timeout_has_safe_floor_and_cap() -> None:
