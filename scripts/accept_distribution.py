...
def main() -> int:
    with TemporaryDirectory(prefix="jobpilot-distribution-acceptance-") as temp_dir:
        root = Path(temp_dir)
        ...
        try:
            ...
            controller.database.set_json_setting("distribution_acceptance", {"value": "before"})
            (paths.master_documents / "distribution.tex").write_text("portable source", encoding="utf-8")
            state = controller.create_local_backup(archive)
            assert archive.is_file()
            assert state["distribution"]["last_backup_created"]["sha256"]
            controller.database.set_json_setting("distribution_acceptance", {"value": "after"})
        ...
        try:
            ...
            assert state["distribution"]["pilot_session_active"] is False
            assert state["distribution"]["real_employer_submission_enabled"] is False
            assert restored.database.get_json_setting("distribution_acceptance") == {"value": "before"}
            assert (paths.models / "machine-specific.bin").read_bytes() == b"preserve"
            assert (paths.master_documents / "distribution.tex").read_text(encoding="utf-8") == "portable source"
