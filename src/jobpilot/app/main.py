...
def create_controller(paths: ManagedPaths | None = None, *, sample_item_seconds: float = 0.4) -> ApplicationController:
    migrations_dir, _ = _resource_paths()
    return ApplicationController(paths or ManagedPaths.default(), migrations_dir, sample_item_seconds=sample_item_seconds)
...
def run_desktop() -> None:
    ...
    controller = ApplicationController(ManagedPaths.default(), migrations_dir)
    bridge = DesktopBridge(controller)
    window = webview.create_window(
        "JobPilot Local",
        url=ui_index.resolve().as_uri(),
