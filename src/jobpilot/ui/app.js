...
function renderModel(model, session, busy) {
  ...
  document.getElementById('runtime-catalogue').innerHTML = model.catalogue.runtimes.map(runtime => {
    ...
    const trust = runtime.trust === 'maintenance_candidate' ? 'maintenance candidate' : 'validated baseline';
    return `<div class="fact-item" data-runtime-id="${escapeAttr(runtime.id)}"><div class="fact-meta"><code>${escapeHtml(runtime.id)}</code><span class="pill">${installed ? 'installed' : 'not installed'}</span></div><div class="fact-source">llama.cpp ${escapeHtml(runtime.version)} / ${escapeHtml(runtime.build)} · ${escapeHtml(runtime.backend)} · ${escapeHtml(trust)} · ${formatBytes(runtime.bytes)} · SHA ${escapeHtml(shortHash(runtime.sha256))}<br>Devices: ${escapeHtml(devices)}${runtime.trust === 'maintenance_candidate' ? '<br>Candidate evaluation never replaces the selected baseline automatically; explicit selection and the review gate remain required.' : ''}</div><div class="button-row"><button data-runtime-action="install" ${idle && !installed ? '' : 'disabled'}>Install verified runtime</button></div></div>`;
  }).join('');
...
function renderTailoring(tailoring, session, busy, resume, model) {
  ...
  document.getElementById('tailoring-gate').innerHTML = selected && gate
    ? `<div><strong>${escapeHtml(tailoring.selected_model_install_id)}</strong><span>Human-approved distinct resumes: ${gate.approved_distinct_resumes}/${gate.required_distinct_resumes}</span><span>Remaining: ${gate.remaining}</span><span>Automatic local tailoring: ${tailoring.auto_tailoring_enabled ? 'enabled' : 'disabled'}</span><span>Job discovery: ${tailoring.job_discovery_enabled ? 'enabled' : 'disabled'}</span><span>Employer submission: disabled</span></div>`
