    ...
    def install(self, artifact: RuntimeArtifact, cancel_event: Event) -> dict[str, Any]:
        ...
        try:
            ...
            self.store.upsert_runtime_install(
                ...
                version=artifact.version,
                backend=artifact.backend,
                install_relpath=str(root.relative_to(self.paths.root)),
                executable_relpath=str(executable.relative_to(self.paths.root)),
            ...
            self.store.upsert_runtime_install(
                ...
                backend=artifact.backend,
                install_relpath=str(root.relative_to(self.paths.root)),
                executable_relpath=str((root / "bin" / artifact.executable_name).relative_to(self.paths.root)),
                artifact_sha256=artifact.sha256,
                status="failed",
